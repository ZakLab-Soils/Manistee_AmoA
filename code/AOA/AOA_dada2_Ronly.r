...
noN.ft <- filterAndTrim(fnRs, fnRs.filtN, maxN=0, multithread = FALSE)
...
for(i in seq_along(fnRs)){system2(cutadapt, args = c("-a", REV,"-a", FWD.RC , "-n", 2, "-o", fnRs.cut[i],fnRs.filtN[i]))}
...
rbind(FWD.ReverseReads = sapply(FWD.orients, primerHits, fn=fnRs.filtN[[1]]), REV.ReverseReads = sapply(REV.orients, primerHits, fn=fnRs.filtN[[1]]))

#                Forward Complement Reverse RevComp
#FWD.ReverseReads       x          x       x       x
#REV.ReverseReads       x          x       x       x

rbind(FWD.ReverseReads = sapply(FWD.orients, primerHits, fn=fnRs.cut[[1]]), REV.ReverseReads = sapply(REV.orients, primerHits, fn=fnRs.cut[[1]]))
...
sample.names <- unname(sapply(cutRs, get.sample.name))
head(sample.names)

filtRs <- file.path(path.cut, "filtered", basename(cutRs))

filter.summary <- filterAndTrim(cutRs, filtRs, maxN = 0, minLen = 200, trimLeft = 5, rm.phix = TRUE, compress = TRUE, multithread = FALSE, maxEE = 2, truncQ = 2)
